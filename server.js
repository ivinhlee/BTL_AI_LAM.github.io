...
async function findAvailablePort(
  ...
  throw new Error(
    `Không tìm được cổng trống từ ${preferredPort} sau ${maxAttempts} lần thử`,
  );
}
...
function runMigrations() {
  ...
  try {
    db.prepare(
      "ALTER TABLE bookings ADD COLUMN num_adults INTEGER DEFAULT 1",
    ).run();
  } catch (err) {
    // Ignore if column already exists
  }
...
async function ensureAdminUser() {
  ...
  if (!canonical && legacy) {
    db.prepare("UPDATE users SET email = ?, is_admin = 1 WHERE id = ?").run(
      canonicalEmail,
      legacy.id,
    );
...
async function startServer() {
  ...
  app.post("/api/register", async (req, res) => {
    try {
      ...
      if (!safeFirstName || !safeEmail || !safePassword) {
        return res
          .status(400)
          .json({ error: "Vui lòng nhập đầy đủ Tên, Email và Mật khẩu" });
      }
      ...
      const stmt = db.prepare(
        "INSERT INTO users (name, email, password, phone, birth_date) VALUES (?, ?, ?, ?, ?)",
      );
  ...
  app.delete(
    ...
    (req, res) => {
      try {
        ...
        res.json({
          message: "Đã xóa toàn bộ booking của phòng",
    ...
    (req, res) => {
      try {
        ...
        if (!room_id || !user_name) {
          return res
            .status(400)
            .json({ error: "room_id và user_name là bắt buộc" });
        }
        ...
        const invalidScore = Object.values(scores).some(
          (score) => Number.isNaN(score) || score < 0 || score > 5,
        );
        if (invalidScore) {
          return res
            .status(400)
            .json({
              error: "Điểm từng tiêu chí phải là số trong khoảng 0 đến 5",
            });
        }
  ...
  app.post("/api/rooms", authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      const safeAmenities = safeAmenitiesList.length
        ? JSON.stringify(safeAmenitiesList)
        : null;
      const safeBookingOptions = booking_options
        ? typeof booking_options === "string"
          ? booking_options
          : JSON.stringify(booking_options)
        : null;
      const safeAccessibility = accessibility
        ...
        : null;
      const safeHostLanguages = host_languages
        ? typeof host_languages === "string"
          ? host_languages
          : JSON.stringify(host_languages)
        : null;
  ...
  app.get("/api/rooms/:id/check-availability", (req, res) => {
    try {
      ...
      const room = db
        .prepare("SELECT id, max_guests FROM rooms WHERE id = ?")
        .get(id);
      if (!room) {
        return res.status(404).json({ error: "Room not found" });
      }
      ...
      if (isNaN(start.getTime()) || isNaN(end.getTime())) {
        return res
          .status(400)
          .json({ error: "Ngày nhận/trả phòng không hợp lệ" });
      }
      ...
      if (startDay < today) {
        return res
          .status(400)
          .json({ error: "Không thể đặt phòng cho ngày trong quá khứ" });
      }
      ...
      if (guests > room.max_guests) {
        return res
          .status(400)
          .json({
            error: `Tối đa ${room.max_guests} khách (không tính thú cưng)`,
          });
      }
      ...
      `;
      const overlap = db
        .prepare(overlapQuery)
        .get(id, check_out_date, check_in_date);
  ...
  app.post("/api/bookings", authenticateToken, (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Vui lòng nhập đầy đủ thông tin đặt phòng" });
      }

      // Calculate total price on the server
      const room = db
        .prepare("SELECT price_per_night, max_guests FROM rooms WHERE id = ?")
        .get(room_id);
      if (!room) {
        return res.status(404).json({ error: "Room not found" });
      }
      ...
      if (totalPeople > room.max_guests) {
        return res
          .status(400)
          .json({
            error: `Tối đa ${room.max_guests} khách (không tính thú cưng)`,
      ...
      if (overlap.count > 0) {
        return res
          .status(409)
          .json({ error: "Rất tiếc, phòng đã được đặt trong thời gian này" });
      }
  ...
  app.post("/api/wishlist", authenticateToken, (req, res) => {
    try {
      ...
      const stmt = db.prepare(
        "INSERT OR IGNORE INTO wishlists (user_id, room_id) VALUES (?, ?)",
      );
  ...
  app.delete("/api/wishlist/:roomId", authenticateToken, (req, res) => {
    try {
      const user_id = req.user.userId;
