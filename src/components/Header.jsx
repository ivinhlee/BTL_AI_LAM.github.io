...
export default function Header() {
  ...
  return (
    <header
      className={`${isRoomDetailPage ? "relative" : "fixed top-0"} w-full z-50 bg-white border-b border-slate-200 transition-all duration-300`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="h-20 flex items-center justify-between">
            <Link
              to="/"
              className={`flex items-center gap-2 group ${isRoomsPage ? "hidden md:flex" : "flex"}`}
            >
            ...
            {user ? (
              <>
                ...
                <Link
                  to="/wishlist"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center gap-3 px-3 py-3 text-base font-medium text-slate-700 hover:bg-slate-50 rounded-lg"
                >
                  <Heart className="w-5 h-5 text-slate-400" /> Danh sách yêu
                  thích
                </Link>
                <Link
                  to="/trips"
