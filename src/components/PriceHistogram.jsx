...
const Range = Slider.Range || (() => null);

export const PriceHistogram = ({ rooms, priceRange, setPriceRange }) => {
  ...
  const { buckets, maxPrice, bucketWidth, minPrice } = useMemo(() => {
    ...
    if (priceRange < 2000000)
      bucketCount = 10; // < 2M: 10 buckets
    else if (priceRange < 5000000)
      bucketCount = 15; // < 5M: 15 buckets
  ...
  const activeRange = useMemo(() => {
    const minBucketIndex = Math.max(
      0,
      Math.floor((priceRange[0] - minPrice) / bucketWidth),
    );
    const maxBucketIndex = Math.min(
      buckets.length - 1,
      Math.ceil((priceRange[1] - minPrice) / bucketWidth),
    );
    ...
    <div className="px-4">
      ...
      <div className="relative h-20 flex items-end gap-1 mb-6">
        {buckets.map((count, idx) => {
          const isActive =
            idx >= activeRange.minBucketIndex &&
            idx <= activeRange.maxBucketIndex;
        ...
        <div className="flex-1 border border-slate-400 rounded-xl px-3 py-2 focus-within:border-black focus-within:border-2 focus-within:p-[7px]">
          ...
          <div className="flex items-center">
            ...
            <input
              ...
              onChange={(e) => {
                const val =
                  parseInt(e.target.value.replace(/[^\d]/g, "")) || maxPrice;
                const clamped = Math.max(
                  Math.min(val, maxPrice),
                  priceRange[0],
                );
