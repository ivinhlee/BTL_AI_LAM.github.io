...
export default function AdminManageReviews() {
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      ...
      if (!roomPickerRef.current.contains(event.target)) {
    ...
    try {
      ...
      if (contentType.includes("application/json")) {
        ...
        if (
          ...
          throw new Error(
            "API backend không phản hồi đúng. Hãy chạy đúng server bằng npm run dev và mở đúng cổng vừa in ra.",
          );
        }
      }
              ...
              <div>
                ...
                <div ref={roomPickerRef} className="relative">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={
                        selectedRoom
                          ? `Đã chọn: ${selectedRoom.title} - ${selectedRoom.location}`
                          : "Tìm theo tên phòng, thành phố, hoặc ID..."
                      }
                  ...
                  <div className="mt-3 flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                    <div>
                      <p className="text-sm font-semibold text-emerald-800">
                        Đang chọn phòng #{selectedRoom.id}
                      </p>
                      <p className="text-sm text-emerald-700">
                        {selectedRoom.title} - {selectedRoom.location}
                      </p>
                    </div>
                    <button
                      type="button"
                ...
                {!loadingRooms && rooms.length === 0 ? (
                  <p className="text-xs text-amber-600 mt-1">
                    Hiện chưa có phòng nào. Bạn cần thêm phòng trước khi tạo
                    review.
                  </p>
                ) : null}
              </div>
              ...
              {RATING_FIELDS.map((field) => (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleScoreChange(field.key, Number(e.target.value))
                    }
