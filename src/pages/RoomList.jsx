...
const getRoomRating = (room) => {
  const raw = room.avg_rating ?? room.rating;
...
export default function RoomList() {
  ...
  const [activeFilters, setActiveFilters] = useState({
    ...
    minGuests: initialGuests ? Number(initialGuests) : "",
    minBeds: initialBeds ? Number(initialBeds) : "",
    minBaths: initialBaths ? Number(initialBaths) : "",
    amenities: [],
    bookingOptions: [],
    hostLanguages: [],
  });
  ...
  useEffect(() => {
    ...
    if (activeFilters.minGuests !== "")
      params.set("guests", String(activeFilters.minGuests));
    if (activeFilters.minBeds !== "")
      params.set("beds", String(activeFilters.minBeds));
    if (activeFilters.minBaths !== "")
      params.set("baths", String(activeFilters.minBaths));
    navigate({ search: params.toString() }, { replace: true });
  }, [
    activeFilters.minGuests,
  ...
  const liveCount = useMemo(() => {
    ...
    return rooms.filter((room) => {
      ...
      const matchRoomType =
        tempFilters.roomType === "Bất kỳ loại nào" ||
        room.room_type === tempFilters.roomType;
      const matchGuests =
        tempFilters.minGuests === "" ||
        room.max_guests >= Number(tempFilters.minGuests);
      const matchBeds =
        tempFilters.minBeds === "" ||
        room.bed_count >= Number(tempFilters.minBeds);
      const matchBaths =
        tempFilters.minBaths === "" ||
        room.bath_count >= Number(tempFilters.minBaths);
      const matchAmenities = tempFilters.amenities.every((a) => {
        const roomAmenities = parseAmenityField(room.amenities);
        return roomAmenities.includes(a);
      });
      const matchBookingOptions = tempFilters.bookingOptions.every((o) => {
        const roomOptions = parseListField(room.booking_options);
        return roomOptions.includes(o);
      });
      const matchHostLanguages = tempFilters.hostLanguages.every((l) => {
        const roomLangs = parseListField(room.host_languages);
  ...
  const filteredRooms = useMemo(() => {
    ...
    return rooms.filter((room) => {
      ...
      const matchRoomType =
        activeFilters.roomType === "Bất kỳ loại nào" ||
        room.room_type === activeFilters.roomType;
      const matchGuests =
        activeFilters.minGuests === "" ||
        room.max_guests >= Number(activeFilters.minGuests);
      const matchBeds =
        activeFilters.minBeds === "" ||
        room.bed_count >= Number(activeFilters.minBeds);
      const matchBaths =
        activeFilters.minBaths === "" ||
        room.bath_count >= Number(activeFilters.minBaths);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("openFilterModal", handleOpenFilter);
  }, [activeFilters]);
  ...
  const toggleTempSelection = (item, listKey) => {
    setTempFilters((prev) => {
        ...
        <div className="w-full">
          ...
          {!loading && error && (
            <div className="flex flex-col items-center justify-center h-64 bg-red-50 rounded-2xl border border-red-100">
              ...
              <p className="text-red-700 font-medium text-center px-4 mb-4">
                {error}
              </p>
              <button
                  ...
                  >
                    ...
                    >
                      <div className="relative aspect-square overflow-hidden bg-slate-200 rounded-3xl">
                        ...
                        >
                          <Heart
                            className={`w-5 h-5 drop-shadow ${wishlistIds.includes(room.id) ? "text-white fill-white" : "text-white/90 stroke-[2] fill-transparent"}`}
                          />
                        </button>
                      </div>
                      ...
                      <div className="mt-3 space-y-1">
                        ...
                        <p className="text-gray-500 text-sm">
                          {room.bed_count} giường • ★ {getRoomRating(room)}
                        </p>
                      </div>
                    </Link>
                  </motion.div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                <div className="pb-8 border-b border-slate-200">
                  ...
                  <p className="text-slate-500 mb-6">
                    Giá chuyến đi, đã bao gồm mọi khoản phí
                  </p>
                  ...
                  <PriceHistogram
                    ...
                    setPriceRange={(range) =>
                      setTempFilters({ ...tempFilters, priceRange: range })
                    }
                  />
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    {["Bất kỳ loại nào", "Phòng", "Toàn bộ nhà"].map((type) => (
                      <button
                        key={type}
                        onClick={() =>
                          setTempFilters({ ...tempFilters, roomType: type })
                        }
                    ...
                    {AMENITIES_LIST.map((amenity) => {
                      ...
                      return (
                        <motion.button
                          key={amenity.key}
                          onClick={() =>
                            toggleTempSelection(amenity.key, "amenities")
                          }
                          whileTap={{ scale: 0.95 }}
                        ...
                        >
                          <Icon
                            className={`w-8 h-8 ${isSelected ? "text-black" : "text-slate-600"}`}
                            strokeWidth={isSelected ? 2 : 1.5}
                          />
                          <span
                            className={`text-sm text-center ${isSelected ? "font-semibold text-black" : "text-slate-600"}`}
                          >
                            {amenity.name}
                          </span>
                        </motion.button>
                  ...
                  <div className="space-y-4">
                    {BOOKING_OPTIONS_LIST.map((option) => (
                      ...
                      >
                        ...
                        <div
                          className={`w-12 h-8 rounded-full transition-colors relative ${tempFilters.bookingOptions.includes(option) ? "bg-black" : "bg-slate-300"}`}
                        >
                          <input
                            type="checkbox"
                            className="sr-only"
                            checked={tempFilters.bookingOptions.includes(
                              option,
                            )}
                            onChange={() =>
                              toggleTempSelection(option, "bookingOptions")
                            }
                          />

                          <div
                            className={`absolute top-1 w-6 h-6 rounded-full bg-white transition-all ${tempFilters.bookingOptions.includes(option) ? "left-5" : "left-1"}`}
                          />
                        </div>
                      </label>
                    ))}
