...
export default function RoomDetail() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!roomContentType.includes("application/json")) {
          throw new Error(
            "API phòng không trả về JSON. Vui lòng kiểm tra server backend đang chạy đúng cổng.",
          );
        }
        ...
        const reviewsContentType =
          reviewsResponse.headers.get("content-type") || "";
    ...
    const checkWishlistStatus = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const listed = data.some((r) => r.id === Number(id));
  ...
  useEffect(() => {
    const onOutsideClick = (event) => {
      ...
      if (!bookingPanelRef.current.contains(event.target)) {
  ...
  const checkInDate =
    hasSelectedDates && selection.startDate
      ? format(selection.startDate, "yyyy-MM-dd")
      : "";
  const checkOutDate =
    hasSelectedDates && selection.endDate
  ...
  const normalizedLightboxIndex =
    ((lightboxIndex % allImages.length) + allImages.length) % allImages.length;
  ...
  const overallRating =
    typeof room?.rating_average === "number"
      ? room.rating_average
      : criteriaValues.reduce((sum, item) => sum + item.value, 0) /
        REVIEW_CRITERIA.length;

  const amenities = useMemo(() => parseAmenityKeys(room?.amenities), [room]);
  ...
  const checkAvailability = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/rooms/${room.id}/check-availability?${params.toString()}`,
      );
      const payload = await response.json().catch(() => ({}));

      if (!response.ok) {
        throw new Error(
          payload.error || "Không kiểm tra được tình trạng phòng",
        );
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <section id="anh" className="mb-10">
          <div className="flex items-start justify-between gap-4 mb-6">
            ...
            >
              <Heart
                className={`w-4 h-4 ${isWishlisted ? "fill-emerald-500 text-emerald-500" : ""}`}
              />
              Lưu
            </button>
          </div>
        ...
        <div className="bg-white/95 border-y border-slate-200 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 mb-8">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-8 h-14 text-sm font-semibold text-slate-700">
              <a href="#anh" className="hover:text-slate-900">
                Ảnh
              </a>
              <a href="#tien-nghi" className="hover:text-slate-900">
                Tiện nghi
              </a>
              <a href="#danh-gia" className="hover:text-slate-900">
                Đánh giá
              </a>
              <a href="#vi-tri" className="hover:text-slate-900">
                Vị trí
              </a>
            </div>
          </div>
          ...
          <div className="space-y-12">
            <section className="border-b border-slate-200 pb-10">
              <h2 className="text-2xl font-bold text-slate-900 mb-2">
                Toàn bộ {room.category.toLowerCase()} cho thuê tại{" "}
                {room.location}
              </h2>
              <p className="text-lg text-slate-700">
                {room.max_guests} khách · {room.bed_count} giường ·{" "}
                {room.bath_count} phòng tắm
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-6">
                {amenities.length === 0 ? (
                  <p className="text-slate-500">
                    Chủ nhà chưa cập nhật tiện nghi cho chỗ ở này.
                  </p>
                ) : (
            ...
            <section
              id="danh-gia"
              className="border-b border-slate-200 pb-10 space-y-8"
            >
              <div className="text-center py-4">
                ...
                <p className="mt-2 text-slate-500 text-lg">
                  Dựa trên {reviewStats?.review_count || reviews.length || 0}{" "}
                  lượt đánh giá xác thực.
                </p>
              </div>
                    ...
                    >
                      ...
                      <div className="w-full h-2 rounded-full bg-slate-200 overflow-hidden mb-4">
                        <div
                          ...
                          style={{
                            width: `${Math.max(0, Math.min(100, (criterion.value / 5) * 100))}%`,
                          }}
                        />
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2">
                {reviews.length === 0 ? (
                  <p className="text-slate-500 text-lg col-span-full">
                  ...
                  reviews.map((review) => (
                    <article key={review.id} className="space-y-3">
                      ...
                      <p className="text-slate-700 leading-relaxed text-lg line-clamp-4">
          ...
          <aside className="relative">
            <div className="space-y-4">
              ...
              <div className="p-6 border border-slate-200 rounded-3xl shadow-xl bg-white">
                <div className="mb-5">
                  <p className="text-3xl font-bold text-slate-900 inline">
                  ...
                  <form onSubmit={handleBooking} className="space-y-4">
                    <div ref={bookingPanelRef} className="relative">
                      <div className="rounded-2xl border border-slate-300 overflow-visible bg-white">
                        ...
                        >
                          <div className="p-3 border-r">
                            ...
                            <p className="text-lg font-semibold text-slate-900 mt-1">
                              {hasSelectedDates && selection.startDate
                                ? format(selection.startDate, "d/M/yyyy")
                                : "Thêm ngày"}
                            </p>
                          </div>
                          <div className="p-3">
                            ...
                            <p className="text-lg font-semibold text-slate-900 mt-1">
                              {hasSelectedDates && selection.endDate
                                ? format(selection.endDate, "d/M/yyyy")
                                : "Thêm ngày"}
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          {showGuestPicker && (
                            <div
                              className="absolute left-0 right-0 top-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl z-20 p-4 space-y-4"
                    ...
                    <div className="space-y-2 text-sm text-slate-700">
                      <div className="flex justify-between">
                        <span>
                          {formatPrice(room.price_per_night)} x {nights} đêm
                        </span>
                    ...
                    {isAvailable === true && (
                      <p className="text-sm text-emerald-600 text-center">
                        Phòng còn trống cho lịch bạn chọn.
                      </p>
                    )}
                    {isAvailable === false && (
                      <p className="text-sm text-emerald-600 text-center">
                        Phòng đã kín trong khoảng thời gian này.
                      </p>
                    )}
                  </form>
                )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/90">
          <div className="absolute top-4 right-4 flex items-center gap-4 text-white">
            <span className="text-sm font-semibold">
              {normalizedLightboxIndex + 1}/{allImages.length}
            </span>
            <button
