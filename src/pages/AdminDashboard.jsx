...
export default function AdminDashboard() {
  ...
  const [selectedHostLanguages, setSelectedHostLanguages] = useState([]);
  ...
  const fetchAllBookings = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = isJson
          ? payload.error || "Tải booking thất bại"
          : "Máy chủ không phản hồi JSON (API có đang chạy?)";
        throw new Error(msg);
      }
      if (!isJson) {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {imageUrls
                        ...
                        .map((url, idx) => (
                          <div
                            key={idx}
                            className="relative aspect-video rounded-2xl overflow-hidden bg-slate-100 border border-slate-200"
                          >
                            <img
                              src={url}
                              alt={`Preview ${idx + 1}`}
                              ...
                              onError={(e) => {
                                e.target.src =
                                  "https://placehold.co/800x400?text=Khong+tai+duoc+anh";
                              }}
                              referrerPolicy="no-referrer"
                            />

                            <span className="absolute top-2 right-2 bg-white/80 text-xs font-semibold text-slate-700 px-2 py-1 rounded-lg shadow-sm">
                              #{idx + 1}
                            </span>
                          </div>
                        ))}
                      ...
                      {filteredBookings.map((b) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-slate-700">
                            <div className="font-semibold" title={b.room_name}>
                              {b.room_name}
                            </div>
                            {b.room_location && (
                              <div className="text-xs text-slate-500">
